...
export interface BuildCreativeBlueprintInput {
  intent: CreativeIntentValue; mode: GenerationModeValue; category: string | null; hasProductIntelligence: boolean; isEditTurn: boolean; campaignSceneTransformation: boolean; creativeConcept: string | null; campaignArtDirection: CampaignArtDirection; campaignCommunication: CampaignCommunication; previousCampaignDNA?: PreviousCampaignDNA | null; hasExplicitCreativeDirection?: boolean;
}
...
export function buildCreativeBlueprint(input: BuildCreativeBlueprintInput): CreativeBlueprint {
  ...
  const campaign = input.campaignSceneTransformation || deliverableClass === "SOCIAL_CAMPAIGN_CREATIVE" || deliverableClass === "WEBSITE_BANNER";
  ...
  const executionMode = input.isEditTurn ? (input.intent === "CHANGE_COMPOSITION" || input.intent === "CREATE_SOCIAL" || input.intent === "CREATE_BANNER" ? "FORMAT_ADAPTATION" : "LOCAL_EDIT") : input.mode !== "TEXT_TO_IMAGE" || deliverableClass === "CAMPAIGN_VARIATION" ? "CONTINUATION" : "NEW_CREATION";
  const objective = ecommerce ? "ECOMMERCE_CLARITY" : campaign ? "PRODUCT_DESIRE" : modelInteraction ? "LIFESTYLE_RELEVANCE" : input.isEditTurn ? "REFINEMENT" : "BRAND_COMMUNICATION";
  ...
  return {
    ...
    productTruth: { identitySource: input.hasProductIntelligence ? "REFERENCE_AND_PRODUCT_INTELLIGENCE" : "REFERENCE_ONLY", sourceScenePolicy: input.campaignSceneTransformation ? "DISCARD_FOR_CAMPAIGN" : input.isEditTurn ? "PRESERVE_REQUESTED" : "REPLACEABLE", categoryFocus: photo.detail },
    commercialStrategy: { objective, channel, visualIntensity: ecommerce ? "RESTRAINED" : campaign ? "HIGH" : "ELEVATED", compositionalPriority },
    creativeDirection: { concept, conceptSource: input.creativeConcept ? "SEMANTIC_PLANNING" : concept ? "CAMPAIGN_DNA" : "RESTRAINED_FALLBACK", campaignSceneTransformation: input.campaignSceneTransformation, experimentation: campaign ? "HIGH" : "CONTROLLED" },
    artDirection,
    ...
    qualityIntent: { profile: ecommerce ? "ECOMMERCE" : modelInteraction ? "MODEL_INTERACTION" : input.isEditTurn ? "EDIT" : "CAMPAIGN", priorityDimensions: ecommerce ? ["PRODUCT_FIDELITY", "COMPOSITION", "COMMERCIAL_USEFULNESS"] : modelInteraction ? ["PRODUCT_FIDELITY", "PHYSICAL_INTERACTION", "ANATOMY", "PHOTOGRAPHIC_REALISM"] : ["PRODUCT_FIDELITY", "BRIEF_FIDELITY", "CREATIVE_QUALITY", "ART_DIRECTION", "COMMERCIAL_USEFULNESS"] },
    conversationIntent: { acknowledgementFocus: input.isEditTurn ? "EDIT_CONTINUITY" : campaign ? "CAMPAIGN_TRANSFORMATION" : "PRODUCT_FIDELITY", suggestContinuation: true },
