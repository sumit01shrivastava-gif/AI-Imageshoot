...
Image generation foundation (Phase 3; extended Phases 5–6) — the layer
...
- `brand-style-presets.ts` (Phase 5) — the 6 built-in brand style presets,
  as code constants (never database rows). Phase 6's MODEL_SHOOT reuses
  the same presets (their `modelStyle` field) — no separate catalog.
...
- `identity-validation.server.ts` (Phase 5) — the honest, non-semantic
  ...
  generationType, including Phase 6's MODEL_SHOOT.
...
docs/lifestyle-generation.md for Phases 5–6's lifestyle/model-imagery
design (data model, brand style presets, identity-validation boundary,
batch generation, aspect ratio). No real AI vendor is installed and no
image generation of any kind happens outside the deterministic test
provider — see docs/roadmap.md.
